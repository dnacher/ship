...
public class CartDAO {
  ...
  private final Logger log = LoggerFactory.getLogger(CartDAO.class.getName());
  @Autowired private CartLineRepository cartLineRepository;
  @Autowired private CartHeaderRepository cartHeaderRepository;
  ...
  public Cart getCartByHeaderId(Integer id) throws ShipException {
    ...
    log.info("getCartByHeaderId {0}", id);
    return cart;
  }
  ...
  public Cart saveCart(Cart cart) throws ShipException {
    ...
    this.cartLineRepository
        .saveAll(cart.getCartLines())
        .forEach(
            cartLine -> {
