...
public class CartHeaderController {
    ...
    public List<CartHeader> saveOrderHeaderes(@RequestBody List<CartHeader> cartHeaderes){
        ...
        cartHeaderes.forEach(orderHeader -> {
    ...
    @PutMapping(value = "/{id}")
    public CartHeader updateOrderHeader(@PathVariable Integer id, @RequestBody CartHeader cartHeader){
        String msg = String.format("The OrderHeader Id %s is different from the Url Id", cartHeader.getId());
